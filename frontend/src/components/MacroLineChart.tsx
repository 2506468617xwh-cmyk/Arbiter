  ...
  return (
    <section className="rounded-lg border border-line bg-panel p-5 shadow-soft">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-semibold text-ink">宏观指标走势</h2>
      ...
      <div className="relative mt-4 overflow-x-auto">
        ...
        >
          ...
          {ticks.map((tick) => {
            const y = yFor(tick);
            return <g key={tick}><line x1={margin.left} y1={y} x2={width - margin.right} y2={y} stroke="#eee4d6" /><text x={margin.left - 10} y={y + 4} textAnchor="end" fontSize="11" fill="#746b64">{tick.toFixed(1)}</text></g>;
          })}
