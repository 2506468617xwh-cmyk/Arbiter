...
export default function FundResearchChatPanel({ symbol, useLlm }: { symbol: string; useLlm: boolean }) {
  ...
  const submit = (e: FormEvent) => {
    ...
    askFundChat({ symbol, question, use_llm: useLlm })
      .then(setAnswer)
      .catch((exc) => setError(exc instanceof Error ? exc.message : "问答失败"))
