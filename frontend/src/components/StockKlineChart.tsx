...
import { StockBar } from "../api/client";
...
function StockKlineChart({
  ...
  const data = useMemo(() => {
    const stockRows = normalizeSeries(bars).slice(-260);
    const benchmarkMap = new Map(normalizeSeries(benchmarkBars).map((row) => [row.date, row.value]));
    const rows = stockRows.map((row, index) => {
      const ma20Raw = index >= 19 ? avg(stockRows.slice(index - 19, index + 1).map((item) => item.value)) : null;
      const ma60Raw = index >= 59 ? avg(stockRows.slice(index - 59, index + 1).map((item) => item.value)) : null;
      return {
        ...row,
        ma20: ma20Raw,
        ma60: ma60Raw,
    ...
    return { rows, min: min - padding, max: max + padding };
  }, [bars, benchmarkBars]);

  if (!data.rows.length) {
    return <div className="rounded-lg border border-dashed border-line bg-panel p-6 text-sm text-muted">暂无可绘制个股趋势数据。</div>;
  }
  ...
  const updateActive = (clientX: number, svg: SVGSVGElement) => {
    ...
    setActive({ date: row.date, x: xFor(index), stock: row.value, ma20: row.ma20, ma60: row.ma60, benchmark: row.benchmark });
  };
  ...
  return (
    <section className="rounded-lg border border-line bg-panel p-5 shadow-soft">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-semibold text-ink">个股 vs 大盘走势</h2>
      ...
      <div className="relative mt-4 overflow-x-auto">
        ...
        {active ? (
          <div className="pointer-events-none absolute top-4 rounded-lg border border-line bg-panel p-3 text-xs shadow-soft" style={{ left: Math.min(Math.max(active.x - 18, 12), width - 230) }}>
            ...
            <p className="mt-1 text-muted">个股：{fmt(active.stock)}</p>
...
function normalizeSeries(bars: StockBar[]): Array<{ date: string; value: number }> {
  const rows = bars
  ...
  if (!base) return [];
  return rows.map((row) => ({ date: row.date, value: (row.close / base) * 100 }));
}
